...
export const blogCreate = async (req, res)=>{
  ...
  try{
    ...
    if(blogToSave) return sendSuccessResponse(res, 200, "Blog saved successfully");
  } catch(error) {
    logger.error(error);
...
export const blogsMy = async (req, res)=>{
  ...
  try{
    ...
    return sendSuccessResponse(res,200, 'Successfully fetch blogs', {formattedMyBlogs});
  } catch(error){
    logger.error(error);
...
const userId = req.user.id;
  ...
  try{ 
    ...
    if(!blog) return sendErrorResponse(res, 404, 'Blog id not found');
    if(blog.user_id.toString() === userId){
      const result = await BlogSchema.findByIdAndUpdate({_id:blogId}, {text:newText, updatedAt: Date.now()});
      if(result) return sendSuccessResponse(res, 200, 'Blog updated successfully');
    }
