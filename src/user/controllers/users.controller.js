...
import { generateAccessToken, generateRefreshToken} from '../helpers/generateToken.helper.js'
import { 
  // removeRefreshToken,
  storeAccessToken,
  storeTokens,
...
export const userRegister = async (req,res) => {
  ...
  const isUsernameExist = await UserModel.findOne({username: reqData.username});
  if(isUsernameExist) return sendErrorResponse(res, 400, "Username already exists");
...
export const userSaveUser = async (req,res)=>{
  ...
  const cachedUserSecret = JSON.parse(await redisClient.get(`registrationSecret:${email}`));
  if(!cachedUserSecret){
  ...
  if(verified){
    const cachedUserData = JSON.parse( await redisClient.get(`registrationUserInfo:${email}`).catch(err=>logger.error(err)));
    if(!cachedUserData) return sendErrorResponse(res, 500, 'Unable to get the cached info. Please try again');
    console.log(cachedUserData);
    const newUserData = UserModel(cachedUserData.newUserInfo);
    try{
      const dataToSave = await newUserData.save();
      if(dataToSave) return sendSuccessResponse(res, 200," OTP verified successfully as well as user registration successfully")
      sendErrorResponse(res, 400, "Failed to save user.");
    } catch (error){
      logger.error(error);
      sendErrorResponse(res, 500, "Error while saving user.");
    }
    redisClient.del(`registrationSecret:${email}`).catch(err=> logger.error(err));
    redisClient.del(`registrationOtp:${email}`).catch(err=> logger.error(err));
    redisClient.del(`registrationUserInfo:${email}`).catch(err=> logger.error(err));
  }
}
...
export const userLogin = async (req,res) =>{
  ...
  bcrypt.compare(reqData.password, existingUser.password,(err, data)=>{
    if(data){
      ...
      sendSuccessResponse(res, 200, {accessToken: accessToken, refreshToken: refreshToken});
    } else {
      if(!existingUser) return sendErrorResponse(res, 400, "Invalid credentials");
    }
  });
...
export const userHome= async (req,res)=>{
  sendSuccessResponse(res, 200, `Welcome to home page.Hi ${req.user.username}.`);
}
...
export const userGenerateToken = async (req,res) =>{
  ...
  const accessToken = generateAccessToken(userInfo);
  if(!storeAccessToken(accessToken, req.user.id)) return sendErrorResponse(res, 500, "Failed to store access token");
  sendSuccessResponse(res, 200, "New access token created successfully", {accessToken});
}

export const userLogout  = async (req,res) => {
  if(removeTokens(req.user.id)) return sendSuccessResponse(res, 200, 'Logout successfully');
